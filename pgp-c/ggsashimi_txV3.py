...
def read_gtf(f, c, tx,majiq_tx,pgp_tx,mstrand,peaks_tx,peaks_flag,AASeq,JuncSpanCoord):
        ...
        # transcripts['"'+pgp_tx+'"'] = int(pgp_tx.split('-')[1]), int(pgp_tx.split('-')[2]), temp_strand #majiq_tx.split('-')[4]
        # exons.setdefault('"'+pgp_tx+'"', []).append((int(pgp_tx.split('-')[1]), int(pgp_tx.split('-')[1])+1, temp_strand)) #majiq_tx.split('-')[4]))
        # exons.setdefault('"'+pgp_tx+'"', []).append((int(pgp_tx.split('-')[2]), int(pgp_tx.split('-')[2])+1, temp_strand)) #majiq_tx.split('-')[4]))

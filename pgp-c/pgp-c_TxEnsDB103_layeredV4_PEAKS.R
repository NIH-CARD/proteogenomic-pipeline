...
if (command_line_flg==1)
{
  ...
  if (length(args)==0)
  {
    stop("Please provide a 6 columns csv file to find Transcripts for each junction, quitting", call.=FALSE)
  } 
  else 
  {
    ...
    print(paste0(args[1]))
    SpliceData <- read.csv(args[1], header=FALSE)
    #Also read Tx list
    #Tx_list <- read.csv(args[2], header=FALSE)
    Tx_list <- unique(read.csv("principal_tx2_merged_samples.csv", header=FALSE))
...
}else
{
  ...
  setwd("/Volumes/SYEDSHAH/MichaelLab/proteogenomic_pipeline/pgp_multilayer/V3")
  if (file.exists("all_tx_events.csv")) {
    #Delete file if it exists
    file.remove("all_tx_events.csv")
  }
  ...
  #Read Peaks File
  SpliceData <- read.csv("FH.csv", header=FALSE)
  Tx_list <- unique(read.csv("principal_tx2.csv", header=FALSE)) #get unique data
  #also read appris annoation data to get principal 1 isoform
  appr_anno1 <- as.data.frame(read.csv("GRCh38_appris_data.principal.txt",sep="\t", header=FALSE))
  #V1 - hugo_symbol, V2 - ENSG_ID, V3 - TX_ID, V4 - , V2 - PRINCIPAL/ALTERNATE
  colnames(appr_anno1)=c('V1','V2','V3','V4','V5')
...
current_gene="";
tx_lengths <- c()
df_notfound <- data.frame(seqnames=str(''), start=numeric(),end=numeric(),strand=str(''),genename=str(''),junc_type=str(''))
df_zeroutr <- data.frame(seqnames=str(''), start=numeric(),end=numeric(),strand=str(''),genename=str(''),junc_type=str(''))
repeated_entries=0
iPSC_events=0
appris_events=0

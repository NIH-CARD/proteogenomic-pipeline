...
} else
{
  ...
  setwd("/Volumes/SYEDSHAH/MichaelLab/proteogenomic_pipeline/github_final_pgp/phase-c/test_exon1_all")
  #Read Peaks File
  peaks_data <- as.data.frame(read.csv("three_events.csv", header=TRUE))
  #Read corresponding FASTA file
  #ce_fasta_strings <- readLines("small_new_CE_FUSED_AA.fasta")
  ce_fasta_strings <- readLines("synth_6tfi_CE_INCLUSION_FUSED_AA.fasta")
  
  #Also Read majiq csv file of the selected events (Here we concatenate ce_inclusion and extension csv files)
  ce_incl_ext_csv <- readLines("IGV_unique_ce_inclusion.csv")
  
  #Also Read nt seq file ce events
  ce_incl_ext_nt_seq <- readLines("synth_ce_inclusion_fused.transeq_in.fasta")
  
  #Read skiptics FASTA file
  es_fasta_strings <- readLines("synth_6tfi_SKIPTICS_FUSED_AA.fasta")
  #Also Read majiq csv file of the selected events (Here we concatenate ce_inclusion and extension csv files)
  es_csv <- readLines("IGV_unique_skiptics_translated.csv")

  #Also Read nt seq file ce events
  es_nt_seq <- readLines("synth_6tfi_skiptics_fused_transeq_in.fasta")
  
  #Read IR FASTA file
  IR_fasta_strings <- readLines("all.IR_AA.fasta")
  #Also Read majiq csv file of IR events
  IR_csv <- readLines("IR_list.csv")
  
  #Also Read nt seq file for IR events
  IR_nt_seq <- readLines("IR_nt.transeq_in.fasta")
...
for (i in 1:dim(clean_peaks_data)[1])
{
  ...
  print(paste0(aa_seq))
  fasta_id_all=strsplit(clean_peaks_data[i,]$Accession,':')[[1]][1] #get only first fasta_id if an aa appear in more than one events
  print(paste0(fasta_id_all))
  #FIRST CHECK IF 
  ...
  if (length(str_split(fasta_id_all, "_")[[1]])==12) #CRYPTICS (inclusion and extension) #now 12 as an extra event# flag is added
  {
    ...
    for(j in seq(2,length(ce_fasta_strings),2))
    {
      ...
      if (grepl(aa_seq, ce_fasta_strings[j],fixed=T) && grepl(fasta_id, ce_fasta_strings[j-1],fixed=T))
      {
        ...
        if (start_end[1] >=0 && start_end[2]<=uex_rng)  #completely in uex
        {
          flag_event=1
          tot=tot+1
          tupex=tupex+1
        ...
        else if(start_end[1]<=uex_rng && start_end[2]>uex_rng && start_end[2]<=boundary2) #overlaps between up_ex and ce
        {
          flag_event=1
          tot=tot+1
          tupexce=tupexce+1
        ...
        else if (start_end[1]>=uex_rng && start_end[2]<=boundary2) #ce region
        {
          flag_event=1
          tot=tot+1
          tce=tce+1
        ...
        else if(start_end[1]<=boundary2 && start_end[2]>boundary2 && start_end[2]<=boundary3) #overlaps between ce and ds_ex
        {
          flag_event=1
          tot=tot+1
          tcednex=tcednex+1
        ...
        else if (start_end[1]>=boundary2 && start_end[2]<=boundary3) #ds_ex
        {
          flag_event=1
          tot=tot+1
          tdnex=tdnex+1
  ...
  else if (length(str_split(fasta_id_all, "_")[[1]])==10) #SKIPTCS - was 9
  {
    ...
    for(j in seq(2,length(es_fasta_strings),2))
    {
      ...
      #search if aa_seq exists in line j and fasta_id exists in line i-1 if the fasta file
      if (grepl(aa_seq, es_fasta_strings[j],fixed=T) && grepl(fasta_id, es_fasta_strings[j-1],fixed=T))
      {
        category='failed'
        flag=1
        ...
        else if (start_end[1] <=uex_rng && start_end[2]>uex_rng && start_end[2]<=boundary2)  # both up_ex-ds_ex
        {
          flag_event=1
          tot=tot+1
        ...
        else if (start_end[1] >uex_rng && start_end[2]<=boundary2)  #completely in dex
        {
          flag_event=1
          tot=tot+1
          skip_dex=skip_dex+1
  ...
  else if (length(str_split(fasta_id_all, "_")[[1]])==8) #IR was 7
  {

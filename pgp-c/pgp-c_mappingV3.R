...
print(paste0('got: ',dim(peaks_data)[1], ' total PEAKS lines'))

#first get rows with empty accession
empty_accession=peaks_data[which(peaks_data$Accession=="",),]
#save 
if (file.exists("empty_accession.csv") || (dim(empty_accession)[1])>0) {
  #Delete file if it exists
  file.remove("empty_accession.csv")
  print(paste0('out of these: ',dim(empty_accession)[1], ' lines have empty accession'))
...
print(paste0('and: ',dim(clean_peaks_data)[1], ' lines have valid accession'))
...
annotated_es=0
all_skiptics=0
skiptics_nt_notfound=0
skiptics_event_notfound=0
skiptics_strand_issue=0
skip_uex=0
skip_uex_dex=0
skip_dex=0
...
all_cryptics=0
ce_events_debug=0
print(paste0('got: ',dim(clean_peaks_data)[1],' Line from input file'))
for (i in 1:dim(clean_peaks_data)[1])
{
  ...
  else if (length(str_split(fasta_id_all_temp, "_")[[1]])==11) #SKIPTCS - was 10 now 11 extra flag for aa length from exon 1 to upstream exon
  {
    ...
    {
      print(paste0('UNKNOWN STRAND TYPE: ',strand, ' @ Line Number: ',i, ' SO ABANDONING IDENTIFICATION FOR THIS ACCESSION'))
      skiptics_strand_issue=skiptics_strand_issue+1
      next
    }
    if(strand =="plus_") strnd='+'
    if(strand =="minus_") strnd='-'
    ...
    for(j in seq(2,length(es_fasta_strings),2))
    {
      ...
      if (grepl(aa_seq, es_fasta_strings[j],fixed=T) && grepl(fasta_id_all_temp, es_fasta_strings[j-1],fixed=T))
      {
        ...
        if(nt_flg==0)
        {
          ...
          skiptics_nt_notfound=skiptics_nt_notfound+1
          break
        }
        #fasta_id=''
        ...
        #check which range this event lies in
        else if (start_end[1] >=AALengthEx1_upexon && start_end[2]<=uex_rng+AALengthEx1_upexon)  
        {
          flag_event=1
          tot=tot+1
          skip_uex=skip_uex+1
    ...
    if (flag==0) #no record found in nt file
    {
      skiptics_nt_notfound=skiptics_nt_notfound+1
    }
    else if (flag_event==0)
    {
      skiptics_event_notfound=skiptics_event_notfound+1
    }
    else
    #if(flag==1 && flag_event==1)
    {
...
print("********** All Events Stats *************")

print(paste0('Total Input Events: ',Total_Events_Read))
write(paste0("********** All Events Stats *************"),file="PEAKS_STATS.txt",append=TRUE)
write(paste0('Total Input Events: ',Total_Events_Read),file="PEAKS_STATS.txt",append=TRUE)

print(paste0('Total Skiptics Events: ',all_skiptics))
print(paste0(skiptics_strand_issue,': skiptics Events have invalid strand'))
print(paste0('Unknown skiptics Events are: ',skiptic_unknown_category))


print(paste0('Skiptics Events for which nt-seq do not match: ',skiptics_nt_notfound))
print(paste0('Skiptics Events for which event_type not found: ',skiptics_event_notfound))

print(paste0('Total Cryptics Events: ',all_cryptics))
...
print(paste0('All Undecided Events (including skpiptics, cryptics and IR) Are: ',ce_unknown_category+skiptic_unknown_category+ir_unknown_category))
...
print(paste0('ce_events debug are: ',ce_events_debug))

print("********** Annotated CRYPTICS and SKIPTICS STATS *************")
...
print(paste0('Total Annotated Skiptics are: ',annotated_es))
...
print(paste0('Finally Total Novel Processed Events are: ',tot))
write(paste0('Finally Total Novel Processed Events are: ',tot),file="PEAKS_STATS.txt",append=TRUE)

...
setwd("/Volumes/SYEDSHAH/MichaelLab/DESeq2/rbp_combined")
#if (!requireNamespace("BiocManager", quietly = TRUE))
#  install.packages("BiocManager")
#BiocManager::install("DESeq2")
library(DESeq2)
# generate dds object by loading count matrix geenrated from stringtie2 (after merging and re-estimation)
countData <- read.csv("/Volumes/SYEDSHAH/MichaelLab/DESeq2/rbp_combined/gene_count_matrix.csv")
#now load metadata file
metaData <- read.csv("/Volumes/SYEDSHAH/MichaelLab/DESeq2/rbp_combined/new_samples.csv", sep="\t", row.names=1)
# run dds functions

...
[ -e res_skiptics/IGV_skiptics_uniq.csv ] && rm res_skiptics/IGV_skiptics_uniq.csv

[ -e res_skiptics/FINAL_STATS_SKIPTICS.txt ] && rm res_skiptics/FINAL_STATS_SKIPTICS.txt
echo "########" STATS FROM esV5_layered_CDSV1.sh >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo "######################################" >> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo CALLING TxEnsDB103_layeredV6.R FROM SKIPTICS SCRITP TO GENERATE BED FILES FOR EACH EVENT >> res_skiptics/FINAL_STATS_SKIPTICS.txt
if [ $bed_flg -eq 1 ]
then
Rscript TxEnsDB103_layeredV6.R $inpfile $txfile #$tx_lst
[ -e EnsDB_tx_not_found.csv ] && cp EnsDB_tx_not_found.csv res_skiptics/.
fi
echo BACK FROM TxEnsDB103_layeredV6.R script >> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo "#######################################" >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo ALL EVENTS READ ARE: $total_events >> res_skiptics/FINAL_STATS_SKIPTICS.txt
ensdb_notfound=0
[ -e res_skiptics/EnsDB_tx_not_found.csv ] && ensdb_notfound=$(cat res_skiptics/EnsDB_tx_not_found.csv| wc -l)
echo EVENTS NOT FOUND IN EnsDB are: $ensdb_notfound , PLEASE SEE res_skiptics/EnsDB_tx_not_found.csv file >> res_skiptics/FINAL_STATS_SKIPTICS.txt
all_others=0
...
echo REMAINING EVENTS PROCESSED ARE: $remaining_events_processed >> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo OUT OF THESE $remaining_events_processed events, non_skiptics are: $all_others, PLEASE SEE res_skiptics/IGV_all_others.csv and all_non_skiptics.csv files >> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo TOTAL SKIPTIC EVENTS ARE: $nrecrdst >> res_skiptics/FINAL_STATS_SKIPTICS.txt

if [ $nrecrdst -gt 0 ]
then
multi_es=0
[ -e res_skiptics/IGV_multi_skiptics.csv ] && multi_es=$(cat res_skiptics/IGV_multi_skiptics.csv | wc -l)
echo OF THESE $nrecrdst SKIPTIC EVENTS, MULTI_SKIPTIC EVENTS ARE: $multi_es, PLEASE SEE res_skiptics/IGV_multi_skiptics.csv >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo OUT OF TOTAL $nrecrdst SKIPTIC EVENTS, UNIQUE SKIPTIC EVENTS ARE: $uniq_events, PLEASE SEE res_skiptics/skiptics_unique.bed >> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo AND REPEATED SKIPTIC EVENTS ARE: $rep_events, PLEASE SEE res_skiptics/skiptics_repeated.bed and res_skiptics/IGV_skiptics_repeated.csv >> res_skiptics/FINAL_STATS_SKIPTICS.txt

echo NOW STARTING CDS PHASE LIST
echo "###########">> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo NOW STARTING CDS PHASE LIST >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo EVENTS LIST FOR US EXONS FOR CDS PHASE LIST IS DONE, PLEASE SEE  res_skiptics/skiptics_us_exons.txt >> res_skiptics/FINAL_STATS_SKIPTICS.txt
##########NEW CODE ENDS
fasta_flg=0
...
echo NOW CALLING es_get_orf.R TO IDENTIFY PROTEIN CODING GENES FROM THE LIST OF IDENTIFIED SKIPTICS AND CDS PHASE
echo NOW CALLING es_get_orf.R TO IDENTIFY PROTEIN CODING GENES FROM THE LIST OF IDENTIFIED SKIPTICS AND CDS PHASE >>res_skiptics/FINAL_STATS_SKIPTICS.txt
#Rscript get_orf_cds.R res_skiptics/skiptics_us_exons.txt
Rscript get_orf_cds.R res_skiptics/skiptics_us_exons.txt res_skiptics/FINAL_STATS_SKIPTICS.txt skiptics
...
echo DONE WITH CDS LIST and PHASE FOR SKIPTICS, Please see res_skiptics/protein_coding.bed >>res_skiptics/FINAL_STATS_SKIPTICS.txt
else
	echo SKIPPING ORF CALCULATIONS, PLEASE SET orf_flg flag to 1 to repeat these calculations
	echo SKIPPING ORF CALCULATIONS, PLEASE SET orf_flg flag to 1 to repeat these calculations >> res_skiptics/FINAL_STATS_SKIPTICS.txt
fi
...
echo !!!!!!!! GOT $(cat res_skiptics/non_protein_coding.bed|wc -l)  ZERO_CDS OR NON-PROTEIN_CODING EVENTS >>res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo NOW UPDATING SKIPTICS LIST WITH CDS, Please see non-protein coding list for non cds events res_skiptics/non_protein_coding.bed >>res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo DONE WITH GENERATION VALID CDS bed and csv files for FINAL NT AND AA TRANSLATION, Please see res_skiptics/cds_skiptics_unique.bed, res_skiptics/cds_skiptics_uniq_sashimi.bed and res_skiptics/cds_IGV_unique_skiptics_translated.csv >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo AND NOW STARTING NT and AA TRANSLATION FOR CONCATENATED EXON RANGES >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo GOT FUSED NT and AA FASTA FILE FOR SKIPTICS EVENTS, PLEASE SEE res_skiptics/cds_skiptics_fused_transeq_in.fasta and res_skiptics/cds_SKIPTICS_FUSED_AA.fasta file >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
#Rscript check_aaV4.R res_skiptics/aa.fasta res_skiptics/cds_SKIPTICS_FUSED_AA.fasta res_skiptics/FINAL_STATS_SKIPTICS.txt

Rscript check_aaV4_allFrames.R res_skiptics/aa.fasta res_skiptics/cds_SKIPTICS_FUSED_AA.fasta res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo DONE WITH TRUNCATION OF FASTA LINES @ STOP CODONS, PLEASe SEE res_skiptics/cds_SKIPTICS_FUSED_AA1.fasta file >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo DONE WITH TRUNCATION OF FASTA LINES @ STOP CODONS FOR ALL FRAMES FASTA, PLEASe SEE res_skiptics/AllFrames_SKIPTICS_FUSED_AA1.fasta file >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo NOW REMOVING EMPTY LINES LEFT BY EVENTS WITH STOP CODONS IN THE BEGINNING ""ARHGAP22_chr10"" is one EXAMPLE >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo DONE WITH REMOVING EMPTY LINES GOT $(echo res_skiptics/cds_SKIPTICS_FUSED_AA_Empty.fasta|wc -l) SKIPTICS HAVING STOP CODON AT THE BEGINNING, PLEASE SEE res_skiptics/cds_SKIPTICS_FUSED_AA_Empty.fasta file >> res_skiptics/FINAL_STATS_SKIPTICS.txt
else
...
echo DONE WITH REMOVING EMPTY LINES GOT $(echo res_skiptics/AllFrames_SKIPTICS_FUSED_AA_Empty.fasta|wc -l) SKIPTICS HAVING STOP CODON AT THE BEGINNING, PLEASE SEE res_skiptics/AllFrames_SKIPTICS_FUSED_AA_Empty.fasta file >> res_skiptics/FINAL_STATS_SKIPTICS.txt
else
...
echo STARTED REMOVING AA "<" 8  >> res_skiptics/FINAL_STATS_SKIPTICS.txt
my_sym='>'
while read -r line
do
    a="${line[@]:0:1}"
...
echo DONE WITH REMOVING AA "<" 8, FINAL FILE IS res_skiptics/cds_PEAKS_SKIPTICS_FUSED_AA.fasta  >> res_skiptics/FINAL_STATS_SKIPTICS.txt


#echo THOSE ARE ALL PERTINENT STATISTICS, PLEASE LET US KNOW IF ANY OTHER INFORMATION MIGHT BE USEFUL!!! >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
#final clean up
#rm *.bed
echo AS SKIPTIC EVENTS ARE: $nrecrdst , SO EXITING >> res_skiptics/FINAL_STATS_SKIPTICS.txt
...
echo THOSE ARE ALL PERTINENT STATISTICS, PLEASE LET US KNOW IF ANY OTHER INFORMATION MIGHT BE USEFUL!!! >> res_skiptics/FINAL_STATS_SKIPTICS.txt
echo "########" DONE STATS FROM esV5_layered_CDSV1.sh >> res_skiptics/FINAL_STATS_SKIPTICS.txt
#echo DONE WITH SKIPTICS IDENTIFICATION, PLEASE SEE res_skiptics/FINAL_STATS_SKIPTICS.txt FOR DETAILS !!!!

...
if (command_line_flg==1)
{
  ...
  else 
  {
    ...
    #Tx_list <- unique(read.csv("principal_tx2_6tfi.csv", header=FALSE))
...
}else
{
  ...
  setwd("/Volumes/SYEDSHAH/MichaelLab/proteogenomic_pipeline/github_final_pgp/phase-b/es/cds_only")
  
  #Read Peaks File
  SpliceData <- read.csv("sorted_clean_combined_samples.csv", header=FALSE)
      ...
      #if(length(Tx_name)>0 && length(exonsBy(edb, by = "tx", filter = TxIdFilter(Tx_name)))>0)
      {
            ...
            if(g_dat1$tx_biotype[1]=='protein_coding')
            {
              if (g_dat[1,]$strand=="+") 
              {
                ...
                {
                   if (g_dat[1,]$width > utr5l) #AARS1 has trl5l > exon 1 width
                   {
                      g_dat[1,]$start = g_dat[1,]$start+utr5l
                   }
                  #else
                  #{
              ...
              else
              {
                ...
                if(SpliceData[i,]$V3 <= g_dat1$tx_cds_seq_end[1]) #for -ve strand, ensdb return tx_cds_end as 5' utr, why??
                {
                  ...
                  if (g_dat[1,]$width > utr5l)
                  {
                    g_dat[1,]$end = g_dat[1,]$end-utr5l
                  }
                  #else
                  #{
                  #  print(paste0('=== gene:', SpliceData[i,]$V5, ' @ line: ',i,' has 5 exon width: ',g_dat[1,]$width, ' and 5 utr length: ' ,utr5l))
            ...
            if(g_dat1$tx_biotype[1]!='protein_coding')
            {
              print(paste0('--- Transcript: ',Tx_name,' gene: ', SpliceData[i,]$V5, ' @ line: ',i,' is: ',g_dat1$tx_biotype[1]))
            }

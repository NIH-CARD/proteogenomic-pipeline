...
write(paste0('From get_orf_cds.R: -------------------- Starting processing file: ',args[1], ' with total events: ',dim(cryptic_regions)[1]),file=args[2],append=TRUE)
...
if (file.exists(paste0(folder,"/cryptic_regions_protein_coding.csv"))) {
  #Delete file if it exists
  file.remove(paste0(folder,"/cryptic_regions_protein_coding.csv"))
}

if (file.exists(paste0(folder,"/unique_cryptic_regions_protein_coding.csv"))) {
  #Delete file if it exists
  file.remove(paste0(folder,"/unique_cryptic_regions_protein_coding.csv"))
}


cryptic_regions_protein_coding$PTC = NA_character_
#get unique list only
write(paste0('From get_orf_cds.R: Got total cryptic_regions list with events (before removing repeated ones): ',length(cryptic_regions_protein_coding),' out of total: ',dim(cryptic_regions)[1],' supplied events, Please SEE  ',folder,"/cryptic_regions_protein_coding.csv file"),file=args[2],append=TRUE)
write.table(cryptic_regions_protein_coding,file=paste0(folder,"/cryptic_regions_protein_coding.csv"),append = TRUE, quote=F, sep=",", row.names=F, col.names=F)
cryptic_regions_protein_coding=unique(cryptic_regions_protein_coding)

zero_cds=0
all_frames_flag=1
write(paste0('From get_orf_cds.R: Got total Unique cryptic_regions list with events: ',length(cryptic_regions_protein_coding),' out of total: ',dim(cryptic_regions)[1],' supplied events, Please SEE ',folder,"/unique_cryptic_regions_protein_coding.csv file"),file=args[2],append=TRUE)
write.table(cryptic_regions_protein_coding,file=paste0(folder,"/unique_cryptic_regions_protein_coding.csv"),append = TRUE, quote=F, sep=",", row.names=F, col.names=F)

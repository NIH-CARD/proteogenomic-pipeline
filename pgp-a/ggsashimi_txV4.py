...
def define_options():
        ...
        parser.add_argument("-g", "--gtf",
                ...
                #Added
        parser.add_argument("-Tx", "--ENST",
                help="ENST TxID to display")

        parser.add_argument("-Majiq", "--EVENT",
                help="MAJIQ Event to display")

        parser.add_argument("-MajiqTx", "--BED",
                help="MAJIQ EVENT BED File to display Transcript Track")

        parser.add_argument("-PeaksTx", "--PeaksBED",
                help="PEAKS EVENT BED File to display In Transcript Track")

        parser.add_argument("-PeaksFlg", "--PeaksFlag",
                help="PEAKS EVENT BED File to display In Transcript Track")

        parser.add_argument("-PeaksAA", "--PeaksAASeq",
                help="PEAKS AA Sequence (Now Size of Event in bp) to display In Transcript Track Title")
        parser.add_argument("-PeaksJuncSpanningCoord", "--PeaksJunCoord",
                help="PEAKS Junction Spanning Coordinates for UEX_CEX Or CE_DEX to display In Transcript Track Title")


        parser.add_argument("-PGPTx", "--PGPBED",
                help="PGP Identified EVENT BED File to display Transcript Track")

        parser.add_argument("-ORIG", "--O_EVENT",
                help="Flag to show Original MAJIQ EVENT ONLY Or With ES/CE EVENTS in the sashimi title")


        parser.add_argument("-UEX", "--UPEXON",
                help="Up side exon length")
        parser.add_argument("-DEX", "--DNEXON",
                help="Down side exon length")

        parser.add_argument("-MajiqStrnd", "--MSTRAND",
                help="MAJIQ Strand for majiq track")


        parser.add_argument("-FullTitle", "--FTITLE",
                help="PGP Identified EVENT BED File to display Transcript Track")

        parser.add_argument("-GeneName", "--GEN_NAME",
                help="MAJIQ GENE NAME to display on Sashimi plots")
        ...
        parser.add_argument("--fix-y-scale", default=True, action="store_true", dest = "fix_y_scale",
                help="Fix y-scale across individual signal plots [default=%(default)s]")
        ...
        parser.add_argument("-F", "--out-format", type=str, default="svg", dest="out_format",
...
#def read_gtf(f, c): #changed
#def read_gtf(f, c, tx):
#def read_gtf(f, c, tx,majiq_tx,pgp_tx,mstrand):
#def read_gtf(f, c, tx,majiq_tx,pgp_tx,mstrand,peaks_tx,peaks_flag):
def read_gtf(f, c, tx,majiq_tx,pgp_tx,mstrand,peaks_tx,peaks_flag,AASeq,JuncSpanCoord):

        print("I have Tx: "+tx)
        #print("Also I have Peaks data"+peaks_tx)
        exons = OrderedDict()
        ...
        with gzip.open(f, 'rt') if f.endswith(".gz") else open(f) as openf:
                for line in openf:
                        ...
                        #all_other=tags.strip().split(";")
                        if el_chr != chr:
                                continue
                        if el not in ("transcript", "exon"):
                                continue
                        try:
                                #txid=all_other[1].strip().split()[1]
                                #txid1=txid.split('.')[0]
                                #txid2=txid1.strip('"')
                                #print(txid2)
                                #tid=all_other[]
                                transcript_id = re.findall('transcript_id ("[^"]+")', tags)[0]
                                #print("Original Tx: "+transcript_id.strip())
                                #print(type(transcript_id))
                                #transcript_id=transcript_id1.split(".", 1)[0]
                                #transcript_idt=transcript_id.strip()
                                transcript_id1=transcript_id.partition('.')[0]
                                transcript_id11=transcript_id1.strip('"')
                                #transcript_id1=transcript_id11.replace(' " ' , " ") #i do not know why
                                #print(transcript_id1[0])
                                #print(transcript_id1[1])
                                #print("and tx now is: "+transcript_id1.strip('"'))
                                #islamuddin
                                #gene_name = re.findall('gene_name ("[^"]+")', tags)[0]
                                #print("gene_name is: "+gene_name)
                        except KeyError:
                        ...
                        #if el == "transcript":
                        if el == "transcript" and transcript_id11 == tx: #Added
                                #print('Managed Here')
                                #print('STRAND '+strand)
                                #print('Tx '+transcript_id)
                                temp_strand=strand
                                temp_tx=transcript_id
                                gene_name=re.findall('gene_name ("[^"]+")', tags)[0]
                                #print(type(gene_name.strip().split()[0]))
                                #print("OTx: "+str(tx)+" foundTX: "+transcript_id.partition('.')[0])
                                if (el_end > start and el_start < end): #event (start,end) should lie within TX (el_start,el_end)
                                        transcripts[transcript_id] = max(start, el_start), min(end, el_end), strand
                                else:
                                        print('i got start: '+str(start)+' el_start: '+str(el_start)+' end: '+str(end)+' el_end: '+str(el_end))

                                continue
                        #if el == "exon":
                        if el == "exon" and transcript_id11 == tx: #Added
                                temp_strand=strand
                                temp_tx=transcript_id
        ...
        #NOW ADD MAJIQ EVENT
        #also add MAJIQ event to actual TX
        #exons.setdefault(temp_tx, []).append((int(majiq_tx.split('-')[1]), int(majiq_tx.split('-')[1])+1, temp_strand))
        #exons.setdefault(temp_tx, []).append((int(majiq_tx.split('-')[3]), int(majiq_tx.split('-')[3])+1, temp_strand))
        #print('"'+majiq_tx.split('-')[0]+'"')
        #print('"'+majiq_tx+'"')
        majiq_tx1=majiq_tx.split('-')[0]+":"+majiq_tx.strip().split('-')[1]+'-'+majiq_tx.strip().split('-')[2]+'\nMajiq Junction'

        #convert short dash

        #majiq_tx1=majiq_tx1.replace('-',u"\u2015")
        #majiq_tx1 = re.sub(u"\u2013", "-", majiq_tx1)
        #print('got strand: '+'"'+mstrand+'"')
        #print('got strand: '+temp_strand)
        #print(mstrand.strip())

        transcripts['"'+majiq_tx1+'"'] = int(majiq_tx.split('-')[1]), int(majiq_tx.split('-')[2]), '"'+mstrand+'"' #majiq_tx.split('-')[4]
        exons.setdefault('"'+majiq_tx1+'"', []).append((int(majiq_tx.split('-')[1]), int(majiq_tx.split('-')[1])+1, '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
        exons.setdefault('"'+majiq_tx1+'"', []).append((int(majiq_tx.split('-')[2]), int(majiq_tx.split('-')[2])+1, '"'+mstrand+'"')) #majiq_tx.split('-')[4]))


        #ALSO ADD PEAKS EVENT IF DEFINED
        #print('"'+peaks_tx+'"')
        if peaks_flag == "1":
            peaks_tx1=peaks_tx.split('-')[0]+":"+peaks_tx.strip().split('-')[1]+'-'+peaks_tx.strip().split('-')[2]+':'+AASeq+'\nMS confirmed AA'
            transcripts['"'+peaks_tx1+'"'] = int(peaks_tx.split('-')[1]), int(peaks_tx.split('-')[2]), '"'+mstrand+'"' #majiq_tx.split('-')[4]
            exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[1]), int(peaks_tx.split('-')[2]), '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
            #exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[2]), int(peaks_tx.split('-')[2])+1, '"'+mstrand+'"')) #majiq_tx.split('-')[4]))


        if peaks_flag == "2":
            if mstrand == "+":
                #print("in "+mstrand)
                peaks_tx1=peaks_tx.split('-')[0]+":"+peaks_tx.strip().split('-')[1]+'-'+peaks_tx.strip().split('-')[4]+'\n'+JuncSpanCoord+'\nMS confirmed AA' #+'\n'+AASeq
                transcripts['"'+peaks_tx1+'"'] = int(peaks_tx.split('-')[1]), int(peaks_tx.split('-')[4]), '"'+mstrand+'"' #majiq_tx.split('-')[4]
                exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[1]), int(peaks_tx.split('-')[2]), '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
                #exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[2]), int(peaks_tx.split('-')[2])+1, '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
                #ALSO ADD SECOND EXON
                exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[3]), int(peaks_tx.split('-')[4]), '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
                #exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[4]), int(peaks_tx.split('-')[4])+1, '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
            if mstrand == "-":
                #print("in "+mstrand)
                peaks_tx1=peaks_tx.split('-')[0]+":"+peaks_tx.strip().split('-')[3]+'-'+peaks_tx.strip().split('-')[2]+'\n'+JuncSpanCoord+'\nMS confirmed AA' #+'\n'+AASeq
                transcripts['"'+peaks_tx1+'"'] = int(peaks_tx.split('-')[3]), int(peaks_tx.split('-')[2]), '"'+mstrand+'"' #majiq_tx.split('-')[4]
                exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[3]), int(peaks_tx.split('-')[4]), '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
                #exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[4]), int(peaks_tx.split('-')[4])+1, '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
                #ALSO ADD SECOND EXON
                exons.setdefault('"'+peaks_tx1+'"', []).append((int(peaks_tx.split('-')[1]), int(peaks_tx.split('-')[2]), '"'+mstrand+'"')) #majiq_tx.split('-')[4]))
        ...
        #Also add PGP identified tracks
        if 'pgp_tx' in locals():
            print('"'+pgp_tx.split('-')[0]+'"')
            pgp_tx1=pgp_tx.split('-')[0]+':'+pgp_tx.split('-')[1]+'-'+pgp_tx.split('-')[2]+'\n'+'\npgp_Identified'
            transcripts['"'+pgp_tx1+'"'] = int(pgp_tx.split('-')[1]), int(pgp_tx.split('-')[2]), '"'+mstrand+'"' #majiq_tx.split('-')[4]
            exons.setdefault('"'+pgp_tx1+'"', []).append((int(pgp_tx.split('-')[1]), int(pgp_tx.split('-')[1])+1, temp_strand)) #majiq_tx.split('-')[4]))
            exons.setdefault('"'+pgp_tx1+'"', []).append((int(pgp_tx.split('-')[2]), int(pgp_tx.split('-')[2])+1, temp_strand)) #majiq_tx.split('-')[4]))
        if 'gene_name' not in vars():
            gene_name=tx+": NOT_FOUND"
        return transcripts, exons ,gene_name #.strip().split()[0]
...
def gtf_for_ggplot(annotation, start, end, arrow_bins,majiq_coord):
        arrow_space = int((end - start)/arrow_bins)
        #majiq_event_data=[int(majiq_coord.split('-')[1]),int(majiq_coord.split('-')[3])]
        majiq_event_data=[majiq_coord.split('-')[1],majiq_coord.split('-')[2]]
        print(majiq_event_data)
        #lbl = print('chr'+":"+str(start)+"-"+str(end))
        #"title" =lbl
        s = """
        ...
        #Added
        # s +="""
        # ann_list[['uparrows']]=data.table(start=c(%(start)s),
        #                                 end=c(%(end)s)
        #                                 )
        # """%({
        #     "start":",".join(majiq_coord.split('-')[1]),
        #     "end":",".join(majiq_coord.split('-')[3]),
        #     })
        #end Added
        if annotation["exons"]:
        ...
        if (length(ann_list[['introns']]) > 0) {
                ...
                # #added - up_arrows - islamuddin
                # gtfp=gtfp+ geom_segment(data=ann_list[['uparrows']], aes(x = start, y = 3, xend = start+100, yend = 4),
                #           arrow = arrow(length = unit(0.5, "cm")))
                # gtfp=gtfp+ geom_segment(data=ann_list[['uparrows']], aes(x = end, y = 3, xend = end+100, yend = 4),
                #           arrow = arrow(length = unit(0.5, "cm")))
                #
                # #end added
        ...
        if (length(ann_list[['exons']]) > 0) {
                gtfp = gtfp + geom_segment(data=ann_list[['exons']], aes(x=start, xend=end, y=tx, yend=tx), size=20, alpha=1,color="black")

                # #added - up_arrows - islamuddin
                # gtfp=gtfp+ geom_segment(data=ann_list[['uparrows']], aes(x = start, y = 3, xend = start+2, yend = 4),
                #           arrow = arrow(length = unit(0.5, "cm")))
                # gtfp=gtfp+ geom_segment(data=ann_list[['uparrows']], aes(x = end, y = 3, xend = end+2, yend = 4),
                #           arrow = arrow(length = unit(0.5, "cm")))
                #
                # #end added
        ...
        """ %(start, end)
        #added-islamuddin
        #s +=ggtitle("Plot of length \n by dose") - ggtitle(gene_name) title = gene_name
...
def setup_R_script(h, w, b, label_dict):
        ...
        theme_update(
                plot.title = element_text(color="red", size=14, face="bold.italic"), #added - islamuddin
                plot.margin = unit(c(15,15,15,15), "pt"),
                panel.grid = element_blank(),
                panel.border = element_blank(),
                axis.line = element_line(size=0.5),
                #axis.title.x = element_text(size=14, face="bold.italic"), #element_blank(),
        ...
        #print(paste0('labels are: ',labels))
        density_list = list()
        junction_list = list()
...
#       args.bam = "/nfs/no_backup/rg/epalumbo/projects/tg/work/8b/8b0ac8705f37fd772a06ab7db89f6b/2A_m4_n10_toGenome.bam"
        ...
        if args.gtf:
                ...
                #transcripts, exons, gene_name = read_gtf(args.gtf, args.coordinates) #islamuddin
                #transcripts, exons,gene_name = read_gtf(args.gtf, args.coordinates,args.ENST,args.BED) #islamuddin
                #transcripts, exons,gene_name = read_gtf(args.gtf, args.coordinates,args.ENST,args.BED, args.PGPBED) #islamuddin
                #transcripts, exons,gene_name = read_gtf(args.gtf, args.coordinates,args.ENST,args.BED, args.PGPBED,args.MSTRAND) #islamuddin
                #transcripts, exons,gene_name = read_gtf(args.gtf, args.coordinates,args.ENST,args.BED, args.PGPBED,args.MSTRAND,args.PeaksBED,args.PeaksFlag) #islamuddin -  added PEAKS TRACK
                transcripts, exons,gene_name = read_gtf(args.gtf, args.coordinates,args.ENST,args.BED, args.PGPBED,args.MSTRAND,args.PeaksBED,args.PeaksFlag,args.PeaksAASeq,args.PeaksJunCoord) #islamuddin -  added PEAKS AASeq

                #print("got tx: "+transcripts)
                #print("got exons: "+exons)
                #print("got gene: "+gene_name)
        #also add gene name
        #also add junction id as title - islamuddin
        chr1,strt1,end1 = parse_coordinates(args.coordinates)
        strt1 = int(strt1)+int(args.UPEXON)+1#+50 #remove offsets added in driving script
        end1 = int(end1)-int(args.DNEXON)#-50 #-500
        print('finally got chr: '+gene_name+' start: '+str(strt1)+' end: '+str(end1))

        #mtitle= 'MajiqEvent: '+args.EVENT+"\n"+"Identified: "+gene_name.strip('""')+"-"+chr1+":"+str(strt1)+"-"+str(end1) #+'(+/- 50bp)'
        if args.O_EVENT == "1":
                if len(gene_name.split(':'))>1 and str(gene_name.split(':')[1]) == " NOT_FOUND":
                    mtitle= "Transcript "+gene_name+" For This MAJIQ_EVENT: +/- 1000bp"
                else:
                    #print(gene_name.split(':')[1])
                    mtitle= args.GEN_NAME #"MAJIQ_EVENT: +/- 1000bp"
        else:
            mtitle= "Identified: "+gene_name.strip('""')+"-"+chr1+":"+str(strt1)+"-"+str(end1) #+'(+/- 50bp)'

        #mtitle= 'MajiqEvent: '+args.EVENT+"\n"+"Identified: "+gene_name.strip('""')+"-"+chr1+":"+str(args.UPEXON)+"-"+str(args.DNEXON) #+'(+/- 50bp)'

        #mtitle= "Final Event: "+gene_name.strip('""')+"-"+chr1+":"+str(strt1)+"-"+str(end1)+'(+/- 50bp)'
        #mtitle="Final Event: "+gene_name.strip('""')+"-"+args.FTITLE


        label_dict['title'] = mtitle
        ...
        for strand in bam_dict:
                ...
                if args.gtf:
                        ...
                        R_script += gtf_for_ggplot(annotation, x[0], x[-1], arrow_bins,args.BED)
                ...
                for (bam_index in 1:length(density_list)) {
                        ...
                        gp = gp + labs(y=labels[[id]]) #- chnaged by below
                        #gp = gp + labs(y=labels[[id]])+ xlab('Genomic Coordinate')+theme(axis.title.x = element_text(size=15,face='bold'))
                        if(exists('coord_dict')) {
                                #gp = gp + scale_x_continuous(expand=c(0, 0.25), breaks = breaks_x_shrinked, labels = breaks_x)+ xlab('Genomic Coordinate')+theme(axis.title.x = element_text(margin = margin(b = -100),size=15,face='bold'))
                                 #gp = gp + scale_x_continuous(expand=c(0, 0.25), breaks = breaks_x_shrinked, labels = breaks_x)+ xlab('Genomic Coordinate')+theme(axis.title.x = element_text(margin = margin(b = -10),size=15,face='bold'))
                                 gp = gp + scale_x_continuous(expand=c(0, 0.25), breaks = breaks_x_shrinked, labels = breaks_x)# + xlab('Genomic Coordinate')+theme(axis.title.x = element_text(size=15,face='bold'))
                        } else {
                                #gp = gp + scale_x_continuous(expand=c(0, 0.25))+ xlab('\\n\\nGenomic Coordinate')+ theme(axis.title.x = element_text(margin = margin(b = -100),size=15,face='bold'))
                                #gp = gp + scale_x_continuous(expand=c(0, 0.25))+ xlab('Genomic Coordinate')+ theme(axis.title.x = element_text(margin = margin(b = -10),size=-10,face='bold'))
                                 gp = gp + scale_x_continuous(expand=c(0, 0.25)) #+ xlab('Genomic Coordinate')+ theme(axis.title.x = element_text(size=15,face='bold'))
                        }
                ...
                #               gp = gp + annotation_custom(grob = rectGrob(x=0, y=0, gp=gpar(col="green"), just=c("left","bottom")), j[1], xmid, j[3], ymid)
                        ...
                        gp=gp+ggtitle(labels["title"]) #- ADDED below
                        #gp=gp+ggtitle(labels["title"])+ xlab('Genomic Coordinate')+theme(axis.title.x = element_text(size=15,face='bold'))

                        gpGrob = ggplotGrob(gp);

                        #print(paste0("grob table is: ",grid.ls(gpGrob)))
                        #print(paste0("bam_index is: ",bam_index))
                        ...
                        if (bam_index == 1) {
                                ...
                                #print(paste0('xaxisGrob$heights[8+vs]',xaxisGrob$heights[8+vs],'gpGrob$heights[1]',gpGrob$heights[1]))
                        ...
                        if (bam_index == 2) {
                                ...
                                maxYtextWidth = gpGrob$widths[3+vs];                     # fix problems ggplot2 vs
                                # Extract Title
                                titleGrob <- gtable_filter(gpGrob, "title") #, trim=F)
                                #xaxisGrob$heights[8+vs] = gpGrob$heights[1]              # fix problems ggplot2 vs
                                #x.axis.height = gpGrob$heights[7+vs] + gpGrob$heights[1] # fix problems ggplot2 vs
                                #ADDED - Remove title from second plot
                                kept_names1 = gpGrob$layout$name[gpGrob$layout$name != "title"]
                                gpGrob <- gtable_filter(gpGrob, paste(kept_names1, sep="", collapse="|"), trim=F)
                                #also remove xlabels
                                #kept_names2 = gpGrob$layout$name[gpGrob$layout$name != "xlab"]
                                #gpGrob <- gtable_filter(gpGrob, paste(kept_names2, sep="", collapse="|"), trim=F)
                        }
                ...
                ########ADDED
                     ...
                     # textGrob(
                    #    expression(bold("Genomic Coordinates")), rot = 0,
                    #    gp = gpar(col = "black", fontsize = 20)
                     # )
                    #)
                    #g <- gtable_add_rows(xaxisGrob,unit(2, "null")) # add a column g$widths[6] wide to the right of horizontal position 6
                    #xaxisGrob <- gtable_add_grob(g , grobs = rect,t=2,l = 2,b=5) # now add the rect grob at the new column position to the right of position 6 (i.e. left-most position y, or l= 7, spanning the whole graph)
                    #xaxisGrob = xaxisGrob + xlab('Genomic Coordinate')+theme(axis.title.x = element_text(size=15,face='bold'))
                    #xaxisGrob = arrangeGrob(xaxisGrob,top=textGrob("Sample Title Two"), ncol=1)
                ############
                ...
                ####ADDED
                #print(paste0("grob table is: ",grid.ls(argrobs)))
                #argrobs <- gtable_add_row_space(argrobs, unit(0.4, "cm"))
                ####
